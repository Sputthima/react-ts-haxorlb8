...
const inp = {
  ...
  boxSizing: "border-box", color: T.textPrimary, background: T.white,
};
...
export default function AdminApp({ user, onBack }) {
  ...
  const runAutoSlotsFunction = async () => {
    ...
    if (result.success) setMsg({ type: "ok", msg: `✅ Edge Function: สร้าง ${result.created} slots | ข้าม ${result.skipped}` });
    else setMsg({ type: "err", msg: `❌ ${result.error}` });
    setSaving(false);
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.bg }}>
      ...
      <div style={{
        ...
        color: T.white, padding: "12px 18px",
        ...
        <div style={{ display: "flex", gap: 4, background: "rgba(255,255,255,.15)", borderRadius: 8, padding: 3, marginLeft: 8, flexWrap: "wrap" }}>
          {[["users","👥 Users"],["subcon","🏢 SubCon"],["config","⚙️ Config"],["slots","📅 Slots"],["audit","📜 Audit"]].map(([t, l]) => (
            <button key={t} onClick={() => setTab(t)} style={{
              ...
              color: tab === t ? T.red : "rgba(255,255,255,.8)",
            }}>{l}</button>
          ))}
        </div>
              ...
              <div style={card({ overflow: "hidden" })}>
                ...
                <div style={{ overflowX: "auto" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                    ...
                    <tbody>
                      {subcons.map(sc => (
                        <tr key={sc.id || sc.subcon_code} style={{ borderBottom: `1px solid #f3f4f6`, opacity: sc.active ? 1 : 0.5 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                ...
                <div style={card({ padding: 20 })}>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    ...
                    <button onClick={generateSlots} disabled={saving} style={{ flex: 2, padding: "10px", background: T.gold, color: T.white, border: "none", borderRadius: 10, fontWeight: 700, cursor: "pointer", fontSize: 13, opacity: saving ? 0.6 : 1 }}>
                ...
                <div style={card({ padding: 20 })}>
                  ...
                  <button onClick={runAutoSlotsFunction} disabled={saving}
                    style={{ padding: "9px 20px", background: T.topbarGrad, color: T.white, border: "none", borderRadius: 9, fontWeight: 700, cursor: "pointer", fontSize: 12, opacity: saving ? 0.6 : 1 }}>
                    ⚡ Run Auto-Slots Now
                  </button>
                </div>
                ...
                <div style={card({ padding: 20 })}>
                  ...
                  <button onClick={clearOldSlots} disabled={saving}
                    style={{ padding: "9px 20px", background: T.redBg, color: T.red, border: "none", borderRadius: 9, fontWeight: 700, cursor: "pointer", fontSize: 12, opacity: saving ? 0.6 : 1 }}>
                    🗑 ลบ Slot ที่ผ่านมา
                  </button>
                </div>
              </div>
                ...
                <div style={card({ padding: 16 })}>
                  ...
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "flex-end" }}>
                    ...
                    <div style={{ flex: "0 0 140px" }}>
                      ...
                      <select value={auditFilter.module} onChange={e => setAuditFilter(p => ({ ...p, module: e.target.value }))} style={{ ...sel, fontSize: 12, padding: "7px 10px" }}>
                        <option value="">ทั้งหมด</option>
                        {["GATE","WAREHOUSE","DOCK","BOOKING","ADMIN"].map(m => (
                    ...
                    <div style={{ flex: 1, minWidth: 180 }}>
                      <label style={{ display: "block", fontSize: 11, fontWeight: 700, marginBottom: 4, color: T.textSecond }}>Search (Group / Booking / Remark)</label>
                      <input value={auditFilter.search} onChange={e => setAuditFilter(p => ({ ...p, search: e.target.value }))}
                        placeholder="MN26042801 / BK17... / keyword"
                        onKeyDown={e => { if (e.key === "Enter") loadAudit(auditFilter); }}
                        style={{ ...inp, fontSize: 12, padding: "7px 10px" }} />
                    </div>

                    <button onClick={() => loadAudit(auditFilter)}
                      style={{ ...BTN.primary, padding: "8px 16px", fontSize: 12, alignSelf: "flex-end", whiteSpace: "nowrap" }}>
                      🔍 ค้นหา
                    </button>
                  ...
                  <div style={{ display: "flex", gap: 6, marginTop: 10, flexWrap: "wrap", alignItems: "center" }}>
                    ...
                    {Object.keys(MODULE_COLOR).map(m => {
                      const count = auditRows.filter(r => r.module === m).length;
                      if (!count) return null;
                      return (
                        <span key={m} style={{
                          fontSize: 10, fontWeight: 800, padding: "2px 8px", borderRadius: 999,
                          background: MODULE_COLOR[m] + "18", color: MODULE_COLOR[m],
                          border: `1px solid ${MODULE_COLOR[m]}44`,
                        }}>{m}: {count}</span>
                      );
                    })}
                  </div>
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
                      <thead>
                        <tr style={{ background: T.navy }}>
                          {["เวลา","Module","Action","Target ID","Group","Booking","SubCon","Actor","Remark"].map(h => (
                      ...
                      <tbody>
                        {!auditLoaded ? (
                          <tr><td colSpan={9} style={{ textAlign: "center", padding: 32, color: T.textMuted, fontSize: 12 }}>กด ค้นหา เพื่อโหลด Audit Log</td></tr>
                        ) : auditRows.length === 0 ? (
                          <tr><td colSpan={9} style={{ textAlign: "center", padding: 32, color: T.textMuted, fontSize: 12 }}>ไม่มีข้อมูลตามเงื่อนไขที่เลือก</td></tr>
                        ) : auditRows.map((r, i) => {
                          ...
                          return (
                            <tr key={r.id || i} style={{ background: i % 2 === 0 ? T.white : "#F8FAFF", borderBottom: `1px solid ${T.border}` }}>
                              <td style={{ padding: "7px 10px", color: T.textMuted, whiteSpace: "nowrap", fontFamily: "monospace", fontSize: 10 }}>
                                {r.timestamp
                                  ? new Date(r.timestamp).toLocaleString("th-TH", { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit", second: "2-digit" })
                                  : "—"}
                              </td>
                              ...
                              <td style={{ padding: "7px 10px", fontFamily: "monospace", fontSize: 10, color: T.textMuted }}>{r.booking_id || "—"}</td>
                              ...
                              <td style={{ padding: "7px 10px", color: T.textSecond, fontSize: 10, maxWidth: 220, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={r.remark || ""}>
                                {r.remark || "—"}
                              </td>
                            </tr>
                          );
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(10,20,50,.6)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999, padding: 16 }}>
          <div style={{ background: T.white, borderRadius: 16, padding: 24, width: "100%", maxWidth: 420, boxShadow: "0 20px 60px rgba(0,0,0,.3)" }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button onClick={createUser} disabled={saving} style={{ flex: 2, padding: "10px", background: T.red, color: T.white, border: "none", borderRadius: 10, fontWeight: 700, cursor: "pointer", fontSize: 13, opacity: saving ? 0.6 : 1 }}>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(10,20,50,.6)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999, padding: 16 }}>
          <div style={{ background: T.white, borderRadius: 16, padding: 24, width: "100%", maxWidth: 400, boxShadow: "0 20px 60px rgba(0,0,0,.3)" }}>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
              ...
              <button onClick={createSubcon} disabled={saving} style={{ flex: 2, padding: "10px", background: T.red, color: T.white, border: "none", borderRadius: 10, fontWeight: 700, cursor: "pointer", fontSize: 13, opacity: saving ? 0.6 : 1 }}>
