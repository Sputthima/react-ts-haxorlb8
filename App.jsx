...
const supabase = createClient(
  "https://qgfldtogrjikyfoirknr.supabase.co",
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InFnZmxkdG9ncmppa3lmb2lya25yIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzcxMDE0NTYsImV4cCI6MjA5MjY3NzQ1Nn0.Nk8nGKQNkhTDR7xmp1BX4wJQreebk_Yap_Y3cUtLqYg"
);
...
function Launcher({user, onSelect, onLogout}) {
  ...
  return (
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#0a2a6e,#1e3a8a,#1d4ed8)"}}>
      <div style={{padding:"24px 28px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
        <div>
          <div style={{fontSize:11,color:"rgba(255,255,255,.5)",letterSpacing:3,fontWeight:700,textTransform:"uppercase",marginBottom:4}}>YCH Ladkrabang Plant</div>
          <div style={{fontSize:24,fontWeight:900,color:"#fff"}}>🏭 Dock Management System</div>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <button onClick={onLogout} style={{border:"1px solid rgba(255,255,255,.2)",background:"transparent",color:"#fff",borderRadius:8,padding:"6px 12px",cursor:"pointer",fontSize:12,fontWeight:700}}>Logout</button>
        </div>
      </div>
      <div style={{padding:"0 28px 28px",display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))",gap:12}}>
        {visible.map(app=>(
          <button key={app.id} onClick={()=>onSelect(app.id)}
            style={{background:"rgba(255,255,255,.08)",backdropFilter:"blur(12px)",border:"1px solid rgba(255,255,255,.12)",borderRadius:16,padding:"20px 18px",textAlign:"left",cursor:"pointer",color:"#fff",position:"relative",overflow:"hidden",transition:"all .2s"}}
          ...
          >
            <div style={{position:"absolute",top:0,left:0,right:0,height:3,background:app.color,borderRadius:"16px 16px 0 0"}}/>
            <div style={{fontSize:28,marginBottom:10}}>{app.icon}</div>
            <div style={{fontSize:15,fontWeight:800,marginBottom:3}}>{app.name}</div>
            <div style={{fontSize:10,color:"rgba(255,255,255,.4)",marginBottom:8,fontWeight:700,textTransform:"uppercase",letterSpacing:.5}}>{app.role}</div>
            <div style={{marginTop:12,fontSize:11,color:app.color,fontWeight:700}}>เปิดแอป →</div>
          </button>
        ))}
      </div>
    </div>
